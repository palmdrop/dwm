...
static void toggleinset(const Arg *arg); // Modded to allow for disabling insets
...
/* Inset toggle */
static const Inset* current_inset;
...
clientmessage(XEvent *e)
{
	...
	if (cme->message_type == netatom[NetWMState]) {
        ...
        // PATCH make focusonnetactive patch optional
        if(!focusonnetactive) {
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
        ...
        drw_text(drw, m->ww - tw, 0, sw, bh, 0, stext, 0);
	}
...
static void scratchpad_hide ()
{
	if (selmon -> sel)
	{
        ...
        if (!selmon->sel->isfloating) // My custom code, not part of patch
            togglefloating(NULL);
		focus(NULL);
		arrange(selmon);
	}
...
static void scratchpad_remove ()
{
	if (selmon -> sel && scratchpad_last_showed != NULL && selmon -> sel == scratchpad_last_showed) {
        scratchpad_last_showed->isfloating = 0;
...
moveresize(const Arg *arg) {
    ...
    /* Child window variable added to be able to place cursor in corner of this window */
    Window child;
	Bool xqp = XQueryPointer(dpy, root, &dummy, &child, &msx, &msy, &dx, &dy, &dui);
	resize(c, nx, ny, nw, nh, True);
	...
	if (xqp && ox <= msx && (ox + ow) >= msx && oy <= msy && (oy + oh) >= msy)
	{
        ...
        /* Code modded to place cursor in corner of window.
         * Avoids focus issues.
         */
		XWarpPointer(dpy, None, child, 0, 0, 0, 0, 0, 0);
	}
...
void 
toggleinset(const Arg *arg)
{
    //const Inset* current = &mons->inset;
    const Inset* next;
    if ( current_inset == &default_inset ) {
        next = &alt_inset;
    } else {
        next = &default_inset;
    }
